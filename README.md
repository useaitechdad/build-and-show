...
| [`subagent-context/`](./subagent-context) | Whether sub-agents (each with their own clean context window) actually keep an orchestrator's context small vs one agent reading everything inline — measured on a "find the common root cause across 6 noisy incident logs" task | Sub-agents kept the lead agent's context **83× leaner** (**262** vs **21,781** tokens) and reached the **identical** conclusion — while using slightly **more** total tokens (22,230 vs 21,781). The win is a clean root context that resists rot, **not** fewer tokens | *(add link)* |
| [`advisor-tool-demo/`](./advisor-tool-demo) | Whether Claude's new **advisor tool** (beta) — a cheap executor model consulting an Opus advisor mid-task — actually delivers near-Opus quality at lower cost, measured on a simple coding task and a real agentic SQL-engine build (graded on hidden edge cases) | Across 3 tasks × 3 model tiers it **never improved quality and always cost more**: advisor was a **4.3× tax** on the simple task, and made a Haiku agent **worse** (90% vs **98.8%**) while costing **more than just using Opus**. The idea is sound but it's beta — it pays off only when the cheap model is genuinely outmatched. **$9.29** real spend, reconciled to the bill | *(add link)* |
| [`verification-loops/`](./verification-loops) | Whether making an AI coding agent **check its own work** (vs trusting its answer) makes it reliable — same model, 5 runs each under three modes (trust / run-tests / differential-verify), all graded on **16 hidden tests** the model never sees | Trusting the unchecked answer was a **coin flip** (only **2/5** runs correct); a **run-its-tests loop hit 5/5 (100%)**; a *fancier* differential verifier did **worse** (4/5, one **0/16** blowup where it shipped broken code). The lever is a check the model can **act on** — and the simple one won. *LLM proposes, deterministic code disposes* | *(add link)* |
